...
area1 = ee.FeatureCollection("users/hehao/area8").geometry() # a small area to test my code
...
for i, img_id in enumerate(img_id_list):
    ...
    toa = S2.divide(10000)  # maskS2clouds(S2)
...
def export_img(bandname, scale):
    export = ee.batch.Export.image.toAsset(
        ...
        description='sentinel2_atmcor_export_area8_' + bandname,     # task任务名称
        assetId='users/hehao/' + 'Sentinel2_Corrected_area8_' + bandname,    # 导出影像名称
        scale=scale,
        maxPixels=1e13
    )
    export.start()
...
for band in [['B1',60],['B2',10], ['B3',10], ['B4',10], ['B5',20], ['B6',20],
